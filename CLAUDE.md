...
Global engineering defaults (SOLID, layer separation, opportunistic refactoring, naming, commit style, tracking-file discipline) live in `~/.claude/CLAUDE.md` and are not restated here. The bindings below are the CogniDoc-specific specifics.

### Non-negotiable constraints (architectural seams)

- **Provider abstraction** — `BaseLLMProvider` (`utils/llm_providers.py`) and `BaseEmbeddingProvider` (`utils/embedding_providers.py`) are ABCs. All LLM/embedding access goes through them (and the singleton in `utils/llm_client.py`); never call a vendor SDK (`google-genai`, `openai`, `ollama`, `anthropic`) directly from pipeline, retrieval, or agent code.
- **Graph backend** — `GraphBackend` ABC (`graph_backend.py`, Strategy pattern) fronts NetworkX/Kùzu. New graph operations go through the ABC and both implementations (`graph_backend_networkx.py`, `graph_backend_kuzu.py`); don't reach for `networkx`/`kuzu` calls outside the backend classes. `knowledge_graph.py` is the facade above it.
- **Public API surface** — `api.py` (`CogniDoc` class) is the entry point; UI (`cognidoc_app.py`) and CLI (`cli.py`) call through it, not into internals.
- **Config home** — `constants.py` holds paths, thresholds, and model names, overridable via `.env`. Check it before hardcoding any tunable (see the Configuration and Key Constants sections). No parallel config path.
- **Pipeline stage mapping** — the stage modules (`convert_to_pdf.py` … `extract_entities.py`) map 1:1 to the ingestion diagram. Keep that mapping when adding/reordering stages, and keep `--skip-*` flags consistent.

### Do not modify

Concrete paths that are runtime state, secrets, or generated artifacts — never hand-edit or commit:

- `data/` (all of `sources/`, `pdfs/`, `images/`, `detections/`, `processed/`, `chunks/`, `embeddings/`, `indexes/`, `vector_store/`, `cache/`) — gitignored runtime state, including generated `data/indexes/ingestion_manifest.json`.
- `models/` — YOLO weights (`models/YOLOv11/yolov11x_best.pt`, gitignored).
- `.env` — secrets (API keys). Gitignored; `.env.example` is the tracked template.
- `uv.lock` — regenerate via `make lock`, don't hand-edit.
- `.venv/`, `logs/`, `*.log`, `.coverage`, `htmlcov/`, `.docai/`.
- `config/graph_schema.yaml` — generated by `schema-generate`/`--regenerate-schema`; user-tunable but edit deliberately, don't overwrite casually.

### Invariants

- **mypy scope** is `src/cognidoc/` only. Many modules carry `ignore_errors=true` in `[[tool.mypy.overrides]]` (deliberate gradual typing) — don't strip an override to "fix" a module unless you are actually typing it end-to-end.
...
| Layer | Files |
|-------|-------|
| Ingestion stages | `run_ingestion_pipeline.py`, `convert_*.py`, `extract_objects_from_image.py`, `parse_image_with_*.py`, `create_*.py`, `chunk_*.py`, `build_indexes.py`, `extract_entities.py`, `checkpoint.py`, `ingestion_manifest.py` |
| Retrieval / query | `hybrid_retriever.py`, `graph_retrieval.py`, `query_orchestrator.py`, `complexity.py`, `helpers.py`, `agent.py`, `agent_tools.py`, `utils/advanced_rag.py` |
| Graph storage | `knowledge_graph.py`, `graph_backend*.py`, `graph_config.py`, `entity_resolution.py` |
| Providers / models | `utils/llm_providers.py`, `utils/embedding_providers.py`, `utils/llm_client.py`, `constants.py` |
| UI / API / entry | `cognidoc_app.py`, `api.py`, `cli.py`, `setup.py` |
| Tests | `tests/test_<module>.py` (mirror the module) |

### Established facts (don't re-question)

Verified, deliberate choices — treat as settled unless the task is explicitly to change them:

- **No LangChain/LlamaIndex** — direct Qdrant/Ollama integration is intentional (see Key Design Decisions).
- **Editable install breaks on paths with spaces** — install non-editable with `UV_LINK_MODE=copy uv pip install ".[all,dev]"`, and launch via `.venv/bin/python` (not `uv run`, which re-syncs editable). See Commands.
- **Index rebuild is always `recreate=True`** — `Document.id` uses random UUIDs (upsert would duplicate) and BM25 has no incremental mode. Not a bug (see Incremental Ingestion).
- **Qdrant embedded = one client per storage folder** — app and tests cannot run simultaneously against the same data dir; `conftest.py` fixtures serialize access (see Tests).
- **Manifest saved only on successful completion** — a crash re-runs the same files by design (see Incremental Ingestion).
- **Two Gemini-model code paths**: `constants.py` reads `DEFAULT_LLM_MODEL`; `utils/llm_providers.py` runtime selection reads `GEMINI_LLM_MODEL`. Both work but through different paths (see Provider Selection).
